  ...
  return (
    <div ref={bubbleRef} className={`flex gap-3 ${isUser ? 'justify-end' : 'justify-start'}`} style={{ opacity: 0 }}>
      <div className="group relative max-w-[80%] px-4 py-3 text-sm leading-relaxed"
        style={isUser ? {
          background: dark
            ? 'linear-gradient(135deg, rgba(0,255,135,0.15), rgba(14,165,233,0.1))'
            : 'linear-gradient(135deg, rgba(5,150,105,0.12), rgba(2,132,199,0.1))',
          border: dark ? '1px solid rgba(0,255,135,0.2)' : '1px solid rgba(5,150,105,0.25)',
          color: dark ? '#f1f5f9' : '#0f172a',
          borderRadius: '18px 18px 4px 18px',
          boxShadow: dark ? '0 4px 20px rgba(0,255,135,0.08)' : '0 4px 20px rgba(5,150,105,0.1)',
        } : {
          ...
          <>
            <ReactMarkdown
              components={{
                ...
                strong: ({ children }) => (
                  <strong className="font-semibold" style={{ color: accent }}>{children}</strong>
                ),
                ...
                code: ({ inline, children }) =>
                  inline ? (
                    <code className="px-1.5 py-0.5 rounded text-xs font-mono"
                      style={{
                        background: dark ? 'rgba(0,255,135,0.1)' : 'rgba(5,150,105,0.1)',
                        color: accent,
                      }}>{children}</code>
                  ) : (
                    <pre className="p-3 rounded-lg mt-2 mb-2 overflow-x-auto"
                      style={{
                        background: dark ? 'rgba(0,0,0,0.4)' : 'rgba(15,23,42,0.05)',
                        border: dark ? '1px solid rgba(0,255,135,0.15)' : '1px solid rgba(5,150,105,0.2)',
                      }}>
                      <code className="text-xs font-mono" style={{ color: accent }}>{children}</code>
                    </pre>
                  ),
              }}
            >
      ...
      {isUser && (
        <div className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center"
          ...
          <User size={14} style={{ color: `${accent}b3` }} />
        </div>
      )}
    </div>
  );
